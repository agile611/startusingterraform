...
> Un pipeline de Terraform no es "ejecutar `apply` en un servidor": es la garantía de que **lo que se aplica es exactamente lo que alguien revisó**. Eso impone una forma: en la *pull request* se valida y se muestra el plan; al fusionar se vuelve a planificar, alguien aprueba, y se aplica ese plan guardado, no uno nuevo. La identidad la da la plataforma (OIDC), no un secreto copiado; el estado se protege con el lock y con la serialización del propio pipeline ([página 8](index.md#pagina-8)). Esta página construye ese flujo en GitHub Actions, Azure DevOps y GitLab CI, y le da a **Topaz** dos papeles: reproducir las fases en tu máquina y servir de emulador dentro del job de PR para que un `apply` de prueba no necesite ninguna suscripción.
>
> **🎯 Objetivos de aprendizaje**
> - Separar el flujo de PR (calidad, plan, prueba) del de `main` (plan, aprobación, apply del artefacto).
> - Autenticar con OIDC en las tres plataformas y repartir permisos entre una identidad de plan y otra de apply.
> - Usar `-input=false`, `-lock-timeout`, `-detailed-exitcode`, `TF_IN_AUTOMATION` y la concurrencia de la plataforma.
> - Ejecutar Topaz como servicio del pipeline para probar módulos con un `apply` real y sin credenciales.
> - Detectar drift de forma programada y limpiar entornos efímeros por PR ([página 10](index.md#pagina-10)).
>
> **🔷 Requisitos previos.** [Páginas 1](index.md#pagina-1) a 11 completadas y destruidas, `~/tf-st/providers.tf` disponible, `git`, opcionalmente `tflint`, `trivy` y `act`, `az account show --query environmentName -o tsv` → `Topaz`. Para 12.3 a 12.5 en real: un repositorio en la plataforma y la cuenta de estado de la [página 7](index.md#pagina-7).
