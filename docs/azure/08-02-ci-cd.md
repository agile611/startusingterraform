...
> - Detectar drift de forma programada y limpiar entornos efímeros por PR (página 10).
>
> **🔷 Requisitos previos.** Páginas 1 a 11 completadas y destruidas, `~/tf-st/providers.tf` disponible, `git`, opcionalmente `tflint`, `trivy` y `act`, `az account show --query environmentName -o tsv` → `Topaz`. Para 12.3 a 12.5 en real: un repositorio en la plataforma y la cuenta de estado de la página 7.
...
jobs:
  ...
  integracion_topaz:                      # apply REAL contra el emulador: prueba el código de punta a punta sin nube
    ...
    services:
      topaz:
        image: ${{ vars.TOPAZ_IMAGE }}    # la misma imagen con la que arrancaste Topaz en la página 1
        ports: ["8443:8443"]
    steps:
      - uses: actions/checkout@v4
      - uses: hashicorp/setup-terraform@v3
...
cp -r ~/tf-mod/modules/red modules/                       # módulo de la página 9/11
